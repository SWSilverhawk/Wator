...
/** macro per la colorazione dell'output */
#define BLU  "\x1B[34m"
...
typedef struct wator {
  ...
  /** pianeta acquatico */
  planet_t* plan;
  /** matrice aggiornata ad ogni ciclo di update_wator; la cella (x,y) contiene 0 se lo squalo (pesce) 
      non è stato aggiornato durante quel ciclo, 1 altrimenti. */
  int ** update;
} wator_t;

/** struttura che contiene le informazioni di una cella */
struct celle_adiacenti {
  /** tipo della cella */
  cell_t tipo;
  /** riga in cui si trova la cella */
  int riga;
  /** colonna in cui si trova la cella */
  int colonna;
};

/** struttura che rappresenta la coda dei lavori che dovranno essere svolti dai thread worker */
typedef struct _coda {
    /** riga da cui comincia il rettangolo */
    int riga_iniziale;
    /** riga con cui termina il rettangolo */
    int riga_finale;
    /** puntatore al lavoro successivo */
    struct _coda* successivo;
} coda;

/** scambia il valore due interi
 \param (*i,*j) puntatori ai due interi da scambiare
...
/** Calcola il risultato dell'operazione dividendo modulo divisore
 \param dividendo dividendo dell'operazione
 \param divisore divisore dell'operazione
 
 \retval risultato risultato dell'operazione
 */
int modulo (int dividendo, int divisore);

/** crea un array di struct contenente i 4 vicini di una cella
 \param pw puntatore alla struttura wator
 \param vicini array di struct già allocato
 \param (x,y) coordinate del pesce/squalo
 
 La funzione controlla le coordinate della cella di cui si vogliono
 conoscere i vicini ed in base ad esse riempe l'array
 */
